...
// import { useLabEncounter, useOrderConceptByUuid, useOrderConceptsByUuids } from './lab-results.resource';
...
import { useLabEncounter, useOrderConceptByUuid, useOrderConceptsByUuids } from '../../laboratory.resource';
...
const LabResults: React.FC<LabResultsProps> = ({ order, patientUuid: patientUuidProp }) => {
  ...
  const [testResultObs, obsUuids] = useMemo(() => {
    if (!encounter) return [[], []];
    const testObs = encounter.obs?.filter((obs) => obs.order?.uuid === order.uuid) ?? [];
    return [testObs, testObs.map((obs) => obs.concept?.uuid).filter(Boolean)];
  }, [encounter, order.uuid]);
